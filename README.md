...
台帳編集では、先に工事を選び、台帳一覧から編集する台帳を開きます。台帳名や写真配置はいつでも変更できます。スマートフォンでは「写真」「台帳」タブを切り替え、写真を選んでから配置先の枠をタップしてください。

「台帳だけ削除」は台帳の表紙、写真配置、台帳文だけを削除し、写真一覧の写真、JPEG、SHA-256、Storageオブジェクトを変更しません。共有台帳では削除状態を同期するため、`202609100001_ledger_lifecycle.sql`をUI公開前に適用してください。revisionが一致しない場合や未送信変更がある場合は削除を止めます。
...
- 共有台帳の配置・台帳文・削除状態はクラウド同期します。クラウド写真は`complete`だけを一覧へ反映し、原寸未取得時は印刷を止めます。
