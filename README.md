...
工事一覧で「ZIP取込み」と表示された工事だけに「この端末から削除」を表示します。確認画面で対象件数と推定容量を確認し、工事名を完全一致で入力すると削除できます。

- `projects`、`photos`、`photoFiles`、`ledgers`、`imports`、対象写真の送信待ち設定を1つのIndexedDBトランザクションで処理します。
- 途中で失敗した場合はトランザクション全体をロールバックします。
- 同名工事は表示名ではなく内部の工事キーで区別します。
- 対象工事だけに属する取込み履歴は削除し、同じZIPを再度取り込めるようにします。
- 将来の複数工事取込み履歴を検出した場合は、他工事との関連を残したまま対象工事との関連だけを外します。その履歴の`exportId`は重複防止のため維持します。
- クラウド由来またはZIP・クラウド混在の工事、別工事の台帳から参照される写真を含む工事は、この機能では削除できません。
- `cloudFiles`、クラウド接続設定、認証セッション、Supabaseの行・Storageオブジェクトには触れません。
...
別のPC・ブラウザ・ホーム画面版から管理する場合は、工事ごとの「現場管理コード」を使用します。通常の参加コードはメンバー参加専用で、管理者権限にはなりません。
