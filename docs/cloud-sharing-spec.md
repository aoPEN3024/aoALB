...
6. 参加失敗は端末ユーザー単位で、現場IDを変えても15分間に5回までとし、現場IDの存在をエラー文から判別できないようにする。
7. 無効化された所属は参加コードを再入力しても自動復帰させない。参加コードから付与できる権限はeditorまたはviewerだけとし、adminは初期bootstrapまたは既存adminの管理操作だけで付与する。
...
- 初期現場はSQL Editor用の一時処理で作成し、永続的なbootstrap関数や一時権限を残さない
