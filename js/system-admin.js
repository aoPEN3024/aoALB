...
export function initSystemAdminUI() {
  ...
  function renderUsers(users) {
    usersRoot.replaceChildren(...users.map(user => {
      ...
      name.textContent = user.displayName || "名称未設定";
      ...
      if (!user.systemAdmin && user.userId !== currentUserId && user.status !== "deleted") {
        actions.append(actionButton("削除扱いにする", () => {
          const entered = prompt(`写真・台帳・監査記録は保持したまま、この利用者を削除扱いにします。確認のためメールアドレスを入力してください。\n${user.email}`);
  ...
  inviteForm.addEventListener("submit", event => {
    ...
    run("招待メールを送信", async () => {
      await invoke({ action: "invite", displayName, email });
      inviteForm.reset();
    });
  });
