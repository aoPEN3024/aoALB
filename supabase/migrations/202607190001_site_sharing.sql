...
create table public.photo_objects (
  ...
  site_id uuid not null references public.sites(id) on delete cascade,
  bucket_id text not null default 'site-photos',
...
begin
  if p_site_id is null or p_site_id !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then return false; end if;
...
begin
  if v_user is null then return query select null::uuid, null::text, null::text, null::public.site_role, 'auth_required'::text; return; end if;
  select s.* into v_site from public.sites s where s.site_code = upper(trim(p_site_code));
  if not found then return query select null::uuid, null::text, null::text, null::public.site_role, 'invalid_join'::text; return; end if;
  select c.* into v_code from public.site_join_codes c where c.site_id = v_site.id for update;
  if not found then return query select null::uuid, null::text, null::text, null::public.site_role, 'invalid_join'::text; return; end if;
  select a.* into v_attempt from public.site_join_attempts a where a.site_id = v_site.id and a.user_id = v_user for update;
  if found and v_attempt.blocked_until > v_now then
    return query select null::uuid, null::text, null::text, null::public.site_role, 'temporarily_blocked'::text; return;
  end if;
  if char_length(coalesce(p_join_code, '')) < 8 or v_code.code_hash <> extensions.crypt(p_join_code, v_code.code_hash) then
    insert into public.site_join_attempts(site_id, user_id, failed_count, window_started_at, blocked_until)
    values (v_site.id, v_user, 1, v_now, null)
    on conflict (site_id, user_id) do update set
      failed_count = case when public.site_join_attempts.window_started_at < v_now - interval '15 minutes' then 1 else public.site_join_attempts.failed_count + 1 end,
      window_started_at = case when public.site_join_attempts.window_started_at < v_now - interval '15 minutes' then v_now else public.site_join_attempts.window_started_at end,
      blocked_until = case when public.site_join_attempts.window_started_at < v_now - interval '15 minutes' then null when public.site_join_attempts.failed_count + 1 >= 5 then v_now + interval '15 minutes' else null end;
    return query select null::uuid, null::text, null::text, null::public.site_role, 'invalid_join'::text; return;
  end if;
  delete from public.site_join_attempts where site_id = v_site.id and user_id = v_user;
  ...
  on conflict (site_id, user_id) do update set device_name = excluded.device_name, active = true, last_seen_at = v_now;
  ...
  return query select v_site.id, v_site.site_code, v_site.name, (select m.role from public.site_members m where m.site_id = v_site.id and m.user_id = v_user), null::text;
end;
$$;
...
begin
  ...
  if char_length(coalesce(p_new_code, '')) < 8 then raise exception 'join_code_too_short'; end if;
  update public.site_join_codes set code_hash = extensions.crypt(p_new_code, extensions.gen_salt('bf', 10)), grant_role = p_grant_role,
    version = version + 1, changed_by = auth.uid(), changed_at = now() where site_id = p_site_id;
  insert into public.audit_logs(site_id, actor_user_id, action, entity_type, entity_id) values (p_site_id, auth.uid(), 'join_code.rotate', 'site', p_site_id);
  return true;
end;
$$;
...
begin
  ...
  if v_ledger.editing_by is not null and v_ledger.editing_by <> auth.uid() and v_ledger.editing_started_at > now() - interval '10 minutes' then raise exception 'already_editing'; end if;
  update public.ledgers set editing_by = auth.uid(), editing_started_at = now() where id = p_ledger_id returning * into v_ledger;
  return v_ledger;
end;
$$;

create function public.admin_bootstrap_site(p_site_code text, p_name text, p_join_code text, p_admin_user_id uuid, p_device_name text)
returns uuid language plpgsql security definer set search_path = '' as $$
declare v_site_id uuid;
begin
  if char_length(coalesce(p_join_code, '')) < 8 then raise exception 'join_code_too_short'; end if;
  insert into public.sites(site_code, name, created_by) values (upper(trim(p_site_code)), trim(p_name), p_admin_user_id) returning id into v_site_id;
  insert into public.site_join_codes(site_id, code_hash, changed_by) values (v_site_id, extensions.crypt(p_join_code, extensions.gen_salt('bf', 10)), p_admin_user_id);
  insert into public.site_members(site_id, user_id, role, device_name) values (v_site_id, p_admin_user_id, 'admin', left(trim(p_device_name), 80));
  return v_site_id;
end;
$$;

revoke execute on function public.join_site(text, text, text) from public, anon, authenticated;
revoke execute on function public.rotate_site_join_code(uuid, text, public.site_role) from public, anon, authenticated;
revoke execute on function public.set_site_member_active(uuid, uuid, boolean) from public, anon, authenticated;
revoke execute on function public.begin_ledger_edit(uuid, bigint) from public, anon, authenticated;
revoke execute on function public.admin_bootstrap_site(text, text, text, uuid, text) from public, anon, authenticated;
revoke execute on function private.role_rank(public.site_role), private.site_role_for(uuid), private.has_site_role(uuid, public.site_role), private.has_site_role_text(text, public.site_role) from public, anon;
revoke execute on function private.bump_revision() from public, anon, authenticated;
...
grant execute on function public.admin_bootstrap_site(text, text, text, uuid, text) to service_role;
grant execute on function private.role_rank(public.site_role), private.site_role_for(uuid), private.has_site_role(uuid, public.site_role), private.has_site_role_text(text, public.site_role) to authenticated, service_role;
...
grant select, insert, update, delete on public.projects, public.photos, public.photo_objects, public.ledgers, public.ledger_pages, public.ledger_slots to authenticated;
grant select, insert, delete on public.sync_events to authenticated;
grant select on public.audit_logs to authenticated;
revoke all on public.site_join_codes, public.site_join_attempts from anon, authenticated;
